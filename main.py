...
# FastAPI app
app = FastAPI(
    title="Eredivisie TRMNL Service",
    description="Eredivisie standings data for TRMNL devices",
    version="1.0.0"
)
...
async def update_standings():
    ...
    async with EredivisieScraper() as scraper_instance:
        ...
        if scraper_instance.data:
            ...
            logger.error("Failed to update standings")
...
async def root(authorized: bool = Depends(verify_api_key)):
    ...
    return {
        "service": "Eredivisie TRMNL Service",
        "status": "running",
        "last_updated": scraper.data.last_updated if scraper.data else None,
        "teams_count": len(scraper.data.teams) if scraper.data else 0
    }
...
async def get_standings(
    ...
    if not scraper.data:
        raise HTTPException(status_code=503, detail="Standings data not available")
